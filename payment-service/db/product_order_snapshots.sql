...
CREATE TABLE `product_order_snapshots` (
    ...
    `coupon_discount` DECIMAL(15, 3) NOT NULL DEFAULT 0.000 COMMENT '쿠폰 할인 금액',
    `points_used` DECIMAL(15, 3) NOT NULL DEFAULT 0.000 COMMENT '포인트 사용 금액',
    `cash_amount` DECIMAL(15, 3) NOT NULL DEFAULT 0.000 COMMENT '실제 PG 결제 금액 (= totalPrice - couponDiscount - pointsUsed)',
...
COMMENT='주문 스냅샷 (결제 취소 시 재고/포인트 복원용)';
