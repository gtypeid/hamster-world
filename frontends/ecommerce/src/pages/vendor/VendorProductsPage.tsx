...
import { useCreateProduct, useUpdateProduct, useAdjustStock } from '../../hooks/useVendorProducts'
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      if (editingProduct) {
        ...
        console.log('[VendorProductsPage] Creating product with data:', requestData)
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-xl shadow-md p-4">
            ...
            <p className="text-2xl font-bold text-hamster-brown">{vendorProducts.length}개</p>
          </div>
          <div className="bg-white rounded-xl shadow-md p-4">
            <p className="text-sm text-gray-600 mb-1">판매중</p>
            <p className="text-2xl font-bold text-green-600">{vendorProducts.filter(p => p.stock > 0).length}개</p>
          </div>
          <div className="bg-white rounded-xl shadow-md p-4">
            <p className="text-sm text-gray-600 mb-1">품절</p>
            <p className="text-2xl font-bold text-red-600">{vendorProducts.filter(p => p.stock === 0).length}개</p>
          </div>
        </div>
