...
interface VendorCouponEditorProps {
  onSave: (data: MerchantCouponFormData) => void
  onCancel: () => void
}

export function VendorCouponEditor({ onSave, onCancel }: VendorCouponEditorProps) {
  ...
  const [formData, setFormData] = useState<MerchantCouponFormData>(() => {
    ...
    return {
      ...
      validUntil: oneMonthLater.toISOString().slice(0, 16),
    }
  })
    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <section className="bg-purple-50 rounded-xl p-4 space-y-4">
          <h3 className="font-bold text-hamster-brown">유효 기간</h3>
          ...
          <p className="text-xs text-gray-500">
            이 기간 동안만 쿠폰을 사용할 수 있습니다. 기간이 지나면 자동으로 만료됩니다.
          </p>
        </section>
