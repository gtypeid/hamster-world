...
export class EventItem extends TopologyWorldItem {
  constructor(
    ...
    _subscribingConsumerIds: string[] | any = [],
    traceContext?: any
  ) {
    super(traceContext)
    // subscribingConsumerIds는 TopologyWorld에서 설정되지만 실제로는 사용되지 않음
  }

  setSubscribingConsumerIds(_ids: string[]): void {
    // TopologyWorld에서 호출되지만 실제로는 사용되지 않음
  }
